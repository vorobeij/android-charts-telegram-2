...
class PiePointerPopup : ChartPointerPopup {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawText(title, leftBorder + padV, y, paints.paintPointerTitle)
        ...
        items.forEach {
            ...
            canvas.drawText(percents, leftBorder + padV + percentsOffset - paints.paintPointerTitle.measureText(percents), y, paints.paintPointerTitle)
            canvas.drawText(it.chartName, leftBorder + padV + percentsOffset, y, paints.paintPointerName)
            canvas.drawText(it.value, leftBorder + w - padV - r1.width(), y, paints.paintPointerValue)
        }

        if (chartsData.canBeZoomed) {
            arrow.draw(leftBorder + w - padV - arrow.size / 2, 2 * padV, canvas, paints.paintArrow)
        }
    }
